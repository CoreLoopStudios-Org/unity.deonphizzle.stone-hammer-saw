...
## 🔍 Detailed Analysis: Proximity Detection & UI Activation Mechanism

Here is a step-by-step breakdown of how the proximity activation and panel display mechanism functions in the codebase:

### 1. Physics Setup & Trigger Volume Initialization
In the [Start()](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs#L55) method of [ChestOpeningSequence.cs](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs), the trigger collider configuration is dynamically optimized:
* The script grabs the `BoxCollider` attached to the Box.
* It sets `isTrigger = true` to allow other colliders to pass through it instead of physically colliding.
* To make interaction comfortable and reliable for the player, it scales the collider to at least `1.8m` width, `1.5m` height, and `1.8m` depth.
* It offsets the collider center upwards by `0.4m` so it aligns properly with the torso height of the player character.

### 2. Character Trigger Event Detection
The player character `Pangopal_01` is moved using Unity's `CharacterController` component (via [ThirdPersonCharacterController.cs](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ThirdPersonCharacterController.cs)).
* When the character walks inside the Box's trigger zone, Unity's physics engine automatically invokes the [OnTriggerEnter(Collider other)](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs#L93) callback on the [ChestOpeningSequence](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs) component.
* `OnTriggerEnter` performs a verification check:
  ```csharp
  ...
  ```
  This checks if the entering collider or its parent transform structure belongs to `Pangopal_01` (to prevent random decorative physics objects or NPCs from triggering the sequence).

### 3. Chest Animation & Timing
Once the character's identity is verified, the sequence proceeds:
1. `isOpened` is set to `true` to ensure the chest only opens once.
2. A DOTween animation sequence is created:
   * It shakes the chest container to create an anticipation visual effect (`chestBox.DOShakePosition`).
   * It rotates the lid of the chest open (`chestLid.DOLocalRotate`).
   * It triggers the glow/burst VFX particle system (`PlayChestVFXEffects`).
3. To align with the animation timing, it starts a delayed routine `DelayedShowPanel(1.0f)` which waits for **1.0 second** (the duration of the opening animation) before showing the selection panel.

### 4. Resolving and Displaying the Weapon Select Panel
After the 1-second delay, [ShowWeaponSelectPanel()](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs#L154) is called to resolve and activate the UI:
1. **Dynamic Resolution**: If the reference `weaponSelectPanel` is unassigned, it scans the scene structure:
   * First, it searches through all loaded GameObjects using `Resources.FindObjectsOfTypeAll<GameObject>()` to locate the object named `WeoponSelect-Panel-Moiib Squad New`.
   * If that search fails, it falls back to looking for a child transform named `WeoponSelect-Panel-Moiib Squad New` directly under the active scene's root `Canvas`.
2. **Panel Activation**: It sets the panel active using `weaponSelectPanel.SetActive(true)`.
3. **Button Hookup**:
   * It scans the panel for button components.
   * If no buttons are found, it procedurally spawns 5 beautiful, dark-themed buttons representing the weapons ("Mini Saw", "Big Saw", "Hammer", "Mini Stone", "Big Stone").
   * It clears old click actions and assigns [SelectWeapon(index)](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/ChestOpeningSequence.cs#L361) to each button.
4. **Slot Machine Triggering**:
   * It finds the [SlotMachineManager](file:///C:/Users/User/Documents/GitHub/unity.deonphizzle.stone-hammer-saw/Assets/Scripts/Controller/SlotMachineManager.cs) in the scene, calls `ResetAndStartSpin()`, and registers itself to the selection callback so that the final selection is captured if the spin completes.
5. **Countdown Timer**: It initializes a 5-second countdown timer that automatically selects the default weapon (Hammer) if the player takes too long.
